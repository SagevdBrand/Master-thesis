...
val_df <- lapply(paste0(study_1_val_data,val_data_files),readRDS,.GlobalEnv)
...
system.time(for(j in 1:n_sim){
  ...
  data_files <- list.files(path = study_1_data, recursive = T, full.names = F) # get the data path 
  df <- lapply(paste0(study_1_data,data_files),readRDS,.GlobalEnv) # Load the data
  ...
  results_app <- get_app_results(study = s1, df = df)
  
  ## Obtain internal validation estimands ##
  # 10 fold cross-validation
  results_10_cv <- get_cv_results(study = s1, df = df, V = 10)
  
  # 5 fold cross-validation
  results_5_cv <- get_cv_results(study = s1, df = df, V = 5)
  
  # 10X10 fold cross-validation 
  results_10x10_cv <- get_10x10_results(study = s1, df = df, V = 10)
