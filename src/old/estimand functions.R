...
<<<<<<< Updated upstream:R/scripts/estimand functions.R
  if (model == "OLS") {
...
=======
  if (model == "Firth" & pred_selection == "none") {
    ...
    fit_app <- logistf(data = df, formula = y ~., flic = T)
    app_matrix <- model.matrix(object = fit_app$formula, data = df)
    p_app <- 1 / (1 + exp(-app_matrix %*% fit_app$coefficients))
    
  } else if (model == "Firth" & pred_selection == "<0.05") {
    model_form <- as.formula(paste("y ~", paste0(colnames(df[,-which(colnames(df)=="y")]),collapse="+")))
    fit_app <- logistf(formula = model_form, data = df, flic = T)
    fit_app <- logistf::backward(object = fit_app)
    app_matrix <- model.matrix(object = fit_app$formula, data = df)
    p_app <- 1 / (1 + exp(-app_matrix %*% fit_app$coefficients))
    
    ind <- c(1, as.vector(as.numeric(str_extract_all(colnames(app_matrix),"[:digit:]"))) + 1)
    ind <- c(na.omit(ind))
    dgm_par <- dgm_par[ind]
...
>>>>>>> Stashed changes:src/estimand functions.R
  }
...
=======
      model_form <- as.formula(paste("y ~", paste0(colnames(df[,-which(colnames(df)=="y")]),collapse="+")))
      fit <- logistf(formula = model_form, data = df, flic = T) 
      fit <- backward(fit)
      iv_matrix <- model.matrix(object = fit$formula, data = df)
      p <- 1 / (1 + exp(-iv_matrix %*% fit$coefficients))
      
      ind <- c(1, as.vector(as.numeric(str_extract_all(colnames(iv_matrix),"[:digit:]"))) + 1) # HANDLE WITH CARE
      ind <- c(na.omit(ind))
      dgm_par <- dgm_par[ind]
